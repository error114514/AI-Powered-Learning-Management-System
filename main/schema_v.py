...
def schemaName_face_search(request):
    '''
    人脸搜索接口 - 上传人脸照片并搜索匹配的用户
    Face search endpoint - upload face photo and search for matching user
    '''
    if request.method == "OPTIONS":
        return JsonResponse({})
    if request.method in ["POST", "GET"]:
        ...
        try:
            ...
            if user_id:
                logging.info(f"人脸搜索成功，匹配用户ID: {user_id}")
            else:
                logging.info("未找到匹配的人脸")
            
        except Exception as e:
            logging.error(f"人脸搜索失败: {str(e)}")
            msg['code'] = crud_error_code
