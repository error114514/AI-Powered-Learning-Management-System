...
def jiaoshi_register(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            ...
            logger.info(f"教师人脸注册成功，用户ID: {user_id}, 结果: {result}")
            
        except Exception as e:
            # 人脸注册失败，回滚用户创建
            error_msg = str(e)
            logger.error(f"教师人脸注册失败: {error_msg}")
            
            try:
                # 删除刚创建的用户记录
                if user_id:
                    jiaoshi.deletes(jiaoshi, jiaoshi, [user_id])
...
def jiaoshi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是" and req_dict.get("userid"):
            ...
            if tablename!="users" and "userid" in jiaoshi.getallcolumn(jiaoshi,jiaoshi):
                req_dict["userid"] = request.session.get("params").get("id")
