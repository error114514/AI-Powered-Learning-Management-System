        ...
        <div v-else>
          <el-form-item v-if="ruleForm.fengmian" label="封面" prop="fengmian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fengmian.split(',')" :src="item" width="100" height="100">
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="知识要点" prop="zhishiyaodian">
          <file-upload
          tip="点击上传知识要点"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.zhishiyaodian?ruleForm.zhishiyaodian:''"
          @change="zhishiyaodianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.zhishiyaodian" label="知识要点" prop="zhishiyaodian">
            <el-button type="text" size="small" @click="download(ruleForm.zhishiyaodian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
                                    <el-col :span="24">  
...
export default {
  ...
  methods: {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fengmian = true;
            continue;
	            	            ...
	            	            if(o=='zhishiyaodian'){
            this.ruleForm.zhishiyaodian = obj[o];
	    this.ro.zhishiyaodian = true;
            continue;
          }
	            	            if(o=='xuexishipin'){
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `xuexitiandi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xuexitiandiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    back() {
      ...
      this.parent.contentStyleChange();
				...
				this.addEditUploadStyleChange()
                                ...
                                zhishiyaodianUploadChange(fileUrls) {
                this.ruleForm.zhishiyaodian = fileUrls;
				this.addEditUploadStyleChange()
            },
